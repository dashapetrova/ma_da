...
g_plot + geom_histogram(binwidth = 50)
```
...
No, because there's long right tail and some outliers.
...
mono1 <- mono %>% filter(RT>600, abs(RTlog - mean(RTlog)) < sd(RTlog) * 3, !TypeOfStimuli %in% c('FillerUnacceptable', 'FillerAcceptable'))
...
mono1 %>% 
  ...
  geom_col(colour = "black", width=0.9, position = 'dodge') + 
  facet_grid(~WordOrder)+
  labs(title = "Type of Stimuli")
```
...
```{r}
mono1 %>% group_by(AcceptabilityJ, WordOrder) %>% 
  count() %>%
...
t.test(mean_rtlog$diff)
```

*Whould you reject null hypothesis?*

It is possible because p-value is lower than 0.05

*What claim about logarithms of reaction time for Congruent and Incongruent stimuli can you make now?*

There is a difference in population means for Incongruent and congruent stimuli.

*How can you interpret difference with the result of 2.3?*

Tested values are different, so the result is different too.
...
For this purpose Anova can be used, because it is used perform unpaired t-test between 3 and more.

```{r}
aov_tmp <- aov(RTlog ~ TypeOfStimuli, data=incong_rtlog)
summary(aov_tmp)
```
We can claim that there is true difference in logarithm of reaction time for different stimuli.
...
The stimuli pairs 'SubjectiveComment-Material-Shape-Color' and 'SubjectiveComment-Material-Size-Nationality' have statistically significant differences.
...
The second model seems to perform better.
